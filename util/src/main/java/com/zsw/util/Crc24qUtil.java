...
public class Crc24qUtil {

	static final int CRC24Q_TABLE[] = { 0x000000, 0x864CFB, 0x8AD50D, 0x0C99F6, 0x93E6E1, 0x15AA1A, 0x1933EC, 0x9F7F17,
	 ...
	 * @return int类型，需要
	 */
	public static byte[] generatorCrc24q(byte[] buf, int byteLength) {
		byte[] b = new byte[3];
		int crc = 0;
		for (int i = 0; i < byteLength; i++)
			crc = ((crc << 8) & 0xFFFFFF) ^ CRC24Q_TABLE[(crc >> 16) ^ (buf[i] & 0xff)];
		b[0] = (byte) ((crc & 0xff0000) >> 16);
		b[1] = (byte) ((crc & 0xff00) >> 8);
		b[2] = (byte) (crc & 0xff);
		return b;
	}
	 ...
	 * @return 校验成功返回true
	 */
	public static boolean checkRtcm(byte[] buf) {
		boolean result = true;
		byte[] check = generatorCrc24q(buf, buf.length - 3);

		String s = Byte2Hex.Bytes2HexString(check);
		System.out.println(s);
		for (int i = 0; i < 3; i++) {
			if (check[i] != buf[buf.length - 3 + i]) {
				result = false;
				break;
			}
		}
